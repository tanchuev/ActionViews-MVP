...
fun <T> Observable<T>.execute(
    ...
    val observer =
        BaseObserver(onNext, onComplete, onError)
...
fun <T> Flowable<T>.execute(
    ...
    val subscriber =
        BaseSubscriber(onNext, onComplete, onError)
...
fun <T> Maybe<T>.execute(
    ...
    val observer =
        BaseMaybeObserver(onComplete, onSuccess, onError)
    compositeDisposable.add(observer)
    subscribe(observer)
    return this
}
...
fun Completable.execute(
    ...
    val observer =
        BaseCompletableObserver(onComplete, onError)
    compositeDisposable.add(observer)
    subscribe(observer)
    return this
}
